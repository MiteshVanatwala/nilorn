...
import { ExcelExportFieldKey, ExcelExportFieldKeyList } from '../../app/types/types';
...
const ExcelExportModalContent = ({
  ...
  return (
    <form onSubmit={onFormSubmit}>
      <ModalBody>
        <ModalHeading title={t('ExcelExport.ModalTitle')} />
        <Text textAlign={'center'} pb={10} fontWeight={'bold'}>{t('ExcelExport.ModalDesc')}</Text>
        <Stack spacing={2}>
            ...
            <Grid templateColumns="repeat(2, 1fr)" gap="6" key={key}>
              ...
              <GridItem pb={2}>
                <Checkbox
                  key={key}
                  isChecked={value}
                  onChange={() => handleCheckboxChange(key as ExcelExportFieldKey)}
                />
              </GridItem>
            </Grid>
          ))}
        ...
        <Box mt={10} mb={10}>
          <Text mb={1} fontWeight={'bold'}>{t('ExcelExport.FileName')}</Text>
          <Input
            value={fileName}
            onChange={(e) => {
        ...
        <Text mt={2} fontSize="sm">{getUniqueClients} {t('ExcelExport.ProductDevelopmentIncluded')}</Text>
      </ModalBody>
